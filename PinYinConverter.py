...
class PinyinConverter:
    ...
    def convert(self, string):
        ...
        for match in matches:
            replacement = self.get_replacement(match)
    ...
    def get_replacement(self, match):
        ...
        for base, vowel in accent_map.items():
            if base in word:
                vowel_char = vowel[0]
                vowel_num = self.vowels[vowel_char]
                accented_vowel_char = self.pinyin[int(tone)][vowel_num]
                replaced_word = word.replace(base, vowel).replace(vowel_char, accented_vowel_char)
    ...
    def get_accent_map(self):
        ...
        if not hasattr(self, 'accent_map'):
            stars = 'a*i a*o e*i ia* ia*o ie* io* iu* ' + \
                    ...
                    'A* E* I* O* U* Ü* ' + \
                    'a* e* i* o* u* ü*'
            nostars = stars.replace('*', '')
            stars_array = stars.split()
            accent_map = {ref: stars_array[i] for i, ref in enumerate(nostars.split())}
...
result = converter.convert("zhuang1")
print(result)